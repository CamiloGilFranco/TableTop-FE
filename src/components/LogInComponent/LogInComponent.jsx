...
const LogInComponent = ({ setShowLogIn }) => {
  ...
  const handleLogout = () => {
    cookies.remove("token", { maxAge: -1 });
    cookies.remove("name", { maxAge: -1 });
    cookies.remove("last_name", { maxAge: -1 });
    cookies.remove("email", { maxAge: -1 });
    cookies.remove("user_role", { maxAge: -1 });
    dispatch(setUser({}));
  ...
  const isAdmin =
    user.user_role === "restaurantAdmin" || user.user_role === "appAdmin";
      ...
      <div className="log-in-component">
        ...
        <div className="log-in-form">
          <div className="log-in-form-title-box">
            <span className="log-in-main-title">
              {languageSelector(language, "logInTitle")}
            </span>
